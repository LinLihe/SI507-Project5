...
def get_tokens(client_key=CLIENT_KEY, client_secret=CLIENT_SECRET,request_token_url=REQUEST_TOKEN_URL,base_authorization_url=BASE_AUTH_URL,access_token_url=ACCESS_TOKEN_URL,verifier_auto=False):
    ...
    if verifier_auto: # if the input is default (True), like tumblr
        ...
        oauth_resp = oauth_inst.parse_authorization_response(redirect_result) # returns a dictionary -- you may want to inspect that this works and edit accor`dingly
...
def return_for_csv(dictionary):
    if dictionary["tags"] == []:
        tag = None
    else:
        tag = ", ".join(x for x in dictionary["tags"])

    if not dictionary['summary']:
        summary = None
    else:
        summary = dictionary["summary"].strip()
    return '"{}","{}","{}","{}"\n'.format(dictionary["date"],dictionary["post_url"],tag,summary)
...
if __name__ == "__main__":
    ...
    # # Invoke functions
    blog_ident_list = ['peacecorps','pitchersandpoets']
    for n in range (len(blog_ident_list)):
        tumblr_search_baseurl = "http://api.tumblr.com/v2/blog/{}.tumblr.com/posts".format(blog_ident_list[n])
        ...
        cache_sample = CACHE_DICTION[create_request_identifier(tumblr_search_baseurl, tumblr_search_params)]["values"]["response"]["posts"]
        cache_list = []
        for i in cache_sample:
            cache_list.append(i)
        # print (len(cache_list))
        file_name = "cache_{}.csv".format(blog_ident_list[n])
        with open(file_name,"w",encoding ="utf-8") as f:
            f.write('"Date","post_url","tags","summary"\n')
            for i in range (len(cache_list)):
                cont_str = return_for_csv(cache_list[i])
                f.write(cont_str)
